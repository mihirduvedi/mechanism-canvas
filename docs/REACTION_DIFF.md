...
`compare_reached_step({ beforeStateId, afterStateId })` is the fourteenth top-level Site Tool. It is read-only, idempotent, and closed-world. `replay_reached_step` is the fifteenth tool and uses the same reachability gate; see [ELECTRON_FLOW_REPLAY.md](ELECTRON_FLOW_REPLAY.md) for its transient presentation contract.
