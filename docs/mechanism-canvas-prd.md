...
All six fixtures are now `verified` with completed chemistry-and-teaching review metadata and enter the production catalog. The **Practice Compass** adds a deterministic, privacy-local cross-exercise evidence profile derived from checks, hints, and completed steps. Its stable profile revision supports two new Site Tools: `get_learning_profile` reads evidence without exposing authored answers, while `propose_practice_plan` stages an ordered one-to-three-exercise plan. There is deliberately no tool that accepts a plan; only the learner can start or dismiss it in the visible interface. Saved workspaces use storage schema v5 with v4, v3, v2, and v1 migration paths. The current tool count is eighteen. See [PRACTICE_COMPASS.md](PRACTICE_COMPASS.md).
