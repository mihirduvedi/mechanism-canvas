...
The product now addresses a broader, specific problem: visual AI tutors either lack the semantic state required to understand a learner's exact work or receive enough control to take over that work. The learner-owned **Collaboration Contract** makes maximum assistance boundaries executable. Observe, Coach, and Collaborate modes expose 11–21 WebMCP tools; a learner-set hint ceiling limits answer-revealing scaffolds; final commits remain learner-only unless explicitly shared. Changing the contract aborts and republishes the current Site Tool registrations, while the shared store independently rejects forbidden calls with `LEARNER_CONTROLLED`. No Site Tool can change the contract. Saved workspaces use schema v6 with v5–v1 migration paths, and demo mode remains memory-only. See [COLLABORATION_CONTRACT.md](COLLABORATION_CONTRACT.md).

The **Intent-Bound Delegation Session** closes the gap between broad permission and the learner's immediate purpose. The learner grants one tab-local job with a frozen preset, active problem/state scope, and four-, six-, or eight-action budget. Registration becomes the intersection of contract and session; stale cached tools are centrally guarded; human state drift and budget exhaustion collapse discovery to three evidence controls; no Site Tool can create, widen, renew, or end the grant. See [INTENT_BOUND_DELEGATION.md](INTENT_BOUND_DELEGATION.md).
...
All six fixtures are now `verified` with completed chemistry-and-teaching review metadata and enter the production catalog. The **Practice Compass** adds a deterministic, privacy-local cross-exercise evidence profile derived from checks, hints, and completed steps. Its stable profile revision supports two Site Tools: `get_learning_profile` reads evidence without exposing authored answers, while `propose_practice_plan` stages an ordered one-to-three-exercise plan. There is deliberately no tool that accepts a plan; only the learner can start or dismiss it in the visible interface. The later Collaboration Contract, Agent Proof Ledger, and Intent-Bound Delegation releases moved saved workspaces to schema v6 and the catalog to twenty-one adaptive tools. See [PRACTICE_COMPASS.md](PRACTICE_COMPASS.md) and [INTENT_BOUND_DELEGATION.md](INTENT_BOUND_DELEGATION.md).
