...
The product now addresses a broader, specific problem: visual AI tutors either lack the semantic state required to understand a learner's exact work or receive enough control to take over that work. The learner-owned **Collaboration Contract** makes assistance boundaries executable. Observe, Coach, and Collaborate modes expose 9–19 WebMCP tools; a learner-set hint ceiling limits answer-revealing scaffolds; final commits remain learner-only unless explicitly shared. Changing the contract aborts and republishes the current Site Tool registrations, while the shared store independently rejects forbidden calls with `LEARNER_CONTROLLED`. No Site Tool can change the contract. Saved workspaces use schema v6 with v5–v1 migration paths, and demo mode remains memory-only. See [COLLABORATION_CONTRACT.md](COLLABORATION_CONTRACT.md).
