...
### August 30, 2026: proof-carrying tutoring and the Collaboration Contract

The product now addresses a broader, specific problem: visual AI tutors either lack the semantic state required to understand a learner's exact work or receive enough control to take over that work. The learner-owned **Collaboration Contract** makes maximum assistance boundaries executable. Observe, Coach, and Collaborate modes expose 11–21 core WebMCP tools; a learner-set hint ceiling limits answer-revealing scaffolds; final commits remain learner-only unless explicitly shared. Changing the contract aborts and republishes the current Site Tool registrations, while the shared store independently rejects forbidden calls with `LEARNER_CONTROLLED`. No Site Tool can change the contract. Saved workspaces use schema v6 with v5–v1 migration paths, and demo mode remains memory-only. See [COLLABORATION_CONTRACT.md](COLLABORATION_CONTRACT.md).

The **Intent-Bound Delegation Session** closes the gap between broad permission and the learner's immediate purpose. The learner grants one tab-local job with a frozen preset, active problem/state scope, and four-, six-, or eight-action budget. Registration becomes the intersection of contract and session; stale cached tools are centrally guarded; human state drift and budget exhaustion collapse discovery to three core evidence controls plus the lab control when relevant; no Site Tool can create, widen, renew, or end the grant. See [INTENT_BOUND_DELEGATION.md](INTENT_BOUND_DELEGATION.md).

The **Counterfactual Mechanism Lab** lets an agent do real hypothesis work without taking over the learner's draft. A human opens two or three tab-local branches, dynamically adding five tools to the 26-tool complete catalog. The agent can set, deterministically check, compare, and recommend branches through a six-action Explore grant. Lab revisions are separate from the main mechanism revision; active work drifts on human scope change; only a checked valid branch can enter the existing learner-review proposal gate. No Site Tool can start, end, or adopt a lab. See [COUNTERFACTUAL_MECHANISM_LAB.md](COUNTERFACTUAL_MECHANISM_LAB.md).

The **Agent Proof Ledger** closes the remaining gap between learner intent, a registered tool's stated contract, and actual page execution. One central wrapper records every Site Tool callback as a privacy-minimized, memory-only receipt with outcome, active contract, optional fixed-structure delegation evidence, bounded semantic IDs, and before/after state stamps. The same receipts appear in a visible learner surface, an explicit JSON export, and the read-only `get_agent_action_receipts` Site Tool. Structured guards, failures, and pre-execution cancellations remain distinct from successful reads, presentations, proposals, and writes. The ledger is capped at 60 tab-local receipts and deliberately omits prompts, rationales, raw inputs and outputs, freeform intent, identity, authored answers, and validation tokens. It does not change the v6 persistence schema. See [AGENT_PROOF_LEDGER.md](AGENT_PROOF_LEDGER.md).

The **WebMCP Live Run Observatory** makes discovery and journey evidence judge-visible without adding another tool. It records a surface only after the host accepts the complete `registerTool` batch, shows exact additions and withdrawals across Contract → Page state → Intent → Host, and explains every dormant capability. Receipt schema version 4 adds only closed-world Lab classifications and counts; a deterministic evaluator then requires seven claims before calling the six-action Explore path complete. Registration refreshes coalesce within one browser task so the host sees the meaningful 16 → 21 → 15 → 4 sequence without transient intermediate batches. Manual projections and failed refreshes remain explicitly outside host proof. See [WEBMCP_LIVE_RUN_OBSERVATORY.md](WEBMCP_LIVE_RUN_OBSERVATORY.md).

The current product thesis is **proof-carrying visual tutoring**: every agent action is tied to a discoverable permission surface, stable semantic objects, a current revision, visible learner gates, deterministic domain validation, and shared provenance. Organic chemistry is the complete flagship implementation; the contract pattern is applicable to other semantic visual workspaces without claiming those editors are already built.

### August 29, 2026: reviewed library and Practice Compass

All six fixtures are now `verified` with completed chemistry-and-teaching review metadata and enter the production catalog. The **Practice Compass** adds a deterministic, privacy-local cross-exercise evidence profile derived from checks, hints, and completed steps. Its stable profile revision supports two Site Tools: `get_learning_profile` reads evidence without exposing authored answers, while `propose_practice_plan` stages an ordered one-to-three-exercise plan. There is deliberately no tool that accepts a plan; only the learner can start or dismiss it in the visible interface. The Collaboration Contract, Agent Proof Ledger, Intent-Bound Delegation, and Counterfactual Lab releases keep saved workspaces at schema v6 while expanding the complete adaptive catalog to twenty-six tools. See [PRACTICE_COMPASS.md](PRACTICE_COMPASS.md), [INTENT_BOUND_DELEGATION.md](INTENT_BOUND_DELEGATION.md), and [COUNTERFACTUAL_MECHANISM_LAB.md](COUNTERFACTUAL_MECHANISM_LAB.md).

The P1 two-step capstone is implemented as the reviewed `ammonia_alkylation_01` fixture. Its SN2 commit advances to a charged methylammonium intermediate; a separately checked proton-transfer commit advances to products. A reached-state timeline and the `view_mechanism_history_state` site tool provide read-only history navigation, future states remain locked, and undo reverses one commit at a time. The capstone is one of the active library's six verified fixtures. The original counts and milestones below remain the build-baseline record; [README.md](../README.md) and [JUDGE_GUIDE.md](JUDGE_GUIDE.md) describe current behavior.

The P1 Learning Record slice is also implemented locally. Each commit can carry a learner-authored reflection that remains attached after undo and does not change chemistry revision or validation authority. A compact instructor view summarizes checks, hint use, active and reversed commits, performed arrow bundles, and reflections. Download JSON and Copy JSON produce an active-exercise-only export from an explicit allowlist; authored accepted bundles, unreached state graphs, validation IDs, dedicated identity fields, pending agent proposals, and other problem workspaces are omitted. Freeform reflections can contain learner-entered text and must be reviewed before sharing. Saved workspaces now use storage schema v6 while preserving v5, v4, v3, v2, and v1 loaders; `?demo=1` remains memory-only. The schema and trust boundary are documented in [LEARNING_RECORD.md](LEARNING_RECORD.md).

The P1 side-by-side comparison is now implemented locally as **Reaction Diff**. Each active committed transition can be inspected as read-only before and after molecular graphs with exact stable-ID bond, formal-charge, lone-pair, and implicit-hydrogen deltas plus the performed arrow bundle. The pure comparison engine powers both the responsive dialog and the read-only `compare_reached_step` Site Tool. Undone transitions and future states are rejected, comparison adds no activity and changes no revision, and the trust and rendering contracts are documented in [REACTION_DIFF.md](REACTION_DIFF.md).

The remaining P1 curved-arrow motion direction is now implemented as **Electron Flow Replay**. A learner can replay the exact performed arrow bundle over the reached before-state from the same evidence dialog; a reduced-motion preference shows the complete bundle without animation. The `replay_reached_step` Site Tool presents that same transient view for one active commit, bringing the current tool count to fifteen. Replay never reapplies chemistry, changes revision, writes persistence, or adds activity, and it remains unavailable for undone or future transitions. The contract is documented in [ELECTRON_FLOW_REPLAY.md](ELECTRON_FLOW_REPLAY.md).

The judge-facing human-agent handoff is strengthened by **Reviewable Agent Proposals**. `propose_draft_arrows` can stage one to four structured arrows plus a brief rationale against the current revision without changing the draft, chemistry, validation, or revision. Only the learner-facing interface can accept or decline the proposal. Acceptance preserves agent authorship on the added arrows, records the learner's decision separately, increments the revision once, and still requires deterministic checking before commit. The persisted proposal and stale-state boundary are documented in [AGENT_DRAFT_PROPOSALS.md](AGENT_DRAFT_PROPOSALS.md).

The P1 problem-library target is implemented. The catalog contains six verified fixtures: three SN2 exercises, two proton-transfer exercises, and the two-step ammonia-alkylation capstone. The added fixtures cover methoxide with bromomethane, methoxide with bromoethane, and methoxide with methylammonium; each includes authored states, accepted arrows, a four-level scaffold, four negative cases, source notes, and a completed review packet. The application, persistence layer, production catalog, and Site Tool state reader all use the shared six-problem catalog. See [PROBLEM_LIBRARY_EXPANSION.md](PROBLEM_LIBRARY_EXPANSION.md).
...
| Chemistry review incomplete | Resolved | High | Review packets were prepared | All six fixtures reviewed on August 29 | Keep fixture review metadata and automated checks separate |
