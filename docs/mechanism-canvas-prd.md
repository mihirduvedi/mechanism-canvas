...
The product now addresses a broader, specific problem: visual AI tutors either lack the semantic state required to understand a learner's exact work or receive enough control to take over that work. The learner-owned **Collaboration Contract** makes assistance boundaries executable. Observe, Coach, and Collaborate modes expose 10–20 WebMCP tools; a learner-set hint ceiling limits answer-revealing scaffolds; final commits remain learner-only unless explicitly shared. Changing the contract aborts and republishes the current Site Tool registrations, while the shared store independently rejects forbidden calls with `LEARNER_CONTROLLED`. No Site Tool can change the contract. Saved workspaces use schema v6 with v5–v1 migration paths, and demo mode remains memory-only. See [COLLABORATION_CONTRACT.md](COLLABORATION_CONTRACT.md).

The **Agent Proof Ledger** closes the remaining gap between a registered tool's stated contract and its actual page execution. One central wrapper records every Site Tool callback as a privacy-minimized, memory-only receipt with outcome, active contract, bounded semantic IDs, and before/after state stamps. The same receipts appear in a visible learner surface, an explicit JSON export, and the twentieth read-only Site Tool, `get_agent_action_receipts`. Structured guards, failures, and pre-execution cancellations remain distinct from successful reads, presentations, proposals, and writes. The ledger is capped at 60 tab-local receipts and deliberately omits prompts, rationales, raw inputs and outputs, identity, authored answers, and validation tokens. It does not change the v6 persistence schema. See [AGENT_PROOF_LEDGER.md](AGENT_PROOF_LEDGER.md).
