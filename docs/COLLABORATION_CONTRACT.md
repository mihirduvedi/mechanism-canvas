...
|---|---:|---|---|
| Observe | 11 | Read and present evidence without changing the exercise or learning record | State, contract, delegation session, agent-action receipts, profile, entity inspection, activity, reached-history view, comparison, replay, focus |
| Coach | 16 | Check learner work and prepare bounded handoffs | Practice plan, arrow proposal, deterministic check, hint request, problem switch |
| Collaborate | 20–21 | Make revision-bound draft edits and reversible state changes | Add/remove arrow, undo, reset; commit only with separate learner opt-in |
...
`get_delegation_session` is also available in every mode. The learner may use the visible page to turn the current contract into a smaller, temporary, purpose-specific grant. A running session freezes its tool set, active problem/state, and action budget; it can shrink but cannot inherit later permission expansion. See [INTENT_BOUND_DELEGATION.md](INTENT_BOUND_DELEGATION.md).
